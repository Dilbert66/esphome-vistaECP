...
empty_sensor_ns = cg.esphome_ns.namespace('alarm_panel')
...
systemstatus= '''[&](std::string statusCode,uint8_t partition) {
      ...
      alarm_panel::publishTextState(&sensor,partition,&statusCode); 
    }'''
line1 ='''[&](std::string msg,uint8_t partition) {
      std::string sensor = "ln1_"+std::to_string(partition);
      alarm_panel::publishTextState(&sensor,partition,&msg);
    }'''
...
beeps='''[&](std::string  beeps,uint8_t partition) {
      ...
      alarm_panel::publishTextState(&sensor,partition,&beeps); 
    }'''
zoneext='''[&](std::string msg) {
      std::string sensor = "zs";
      alarm_panel::publishTextState(&sensor,0,&msg);  
    }'''
lrr='''[&](std::string msg) {
...
statuschange='''[&](alarm_panel::sysState led,bool open,uint8_t partition) {
     std::string sensor="NIL";   
      switch(led) {
                ...
                case alarm_panel::sac: sensor="ac"; alarm_panel::publishBinaryState(&sensor,partition,open);return;       
                case alarm_panel::sbypass: sensor="byp_";break;  
                case alarm_panel::schime: sensor="chm_";break;
                case alarm_panel::sbat: sensor="bat";alarm_panel::publishBinaryState(&sensor,partition,open);return;  
      ...
      alarm_panel::publishBinaryState(&sensor,partition,open);
    }'''
zonebinary='''[&](int zone, bool open) {
      std::string sensor = "z" + std::to_string(zone) ;
      alarm_panel::publishBinaryState(&sensor,0,open);    
    }'''
zonestatus='''[&](int zone, std::string open) {
      std::string sensor = "z" + std::to_string(zone);
      alarm_panel::publishTextState(&sensor,0,&open); 
    }''' 
