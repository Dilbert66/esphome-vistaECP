...
#if !defined(ARDUINO_MQTT)
void publishBinaryState(std::string * str,uint8_t partition,bool open) {
...
void publishTextState(std::string * str,uint8_t partition,std::string * text) {
...
class vistaECPHome: public api::CustomAPIDevice, public time::RealTimeClock {
    ...
    void set_partitionKeypad(uint8_t idx,uint8_t addr) {if (idx && idx<4) partitionKeypads[idx-1]=addr;}
